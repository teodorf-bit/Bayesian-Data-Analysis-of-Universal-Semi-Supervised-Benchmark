...
## Import dataset, remove defixmath because it is only used for task == "usb_small", therefore it cannot be used for the aggregated results.
```{r}
...
```{r include=F}
saveRDS(dplyr::sample_n(dataset,size=4),'./probsuccessmodeldata-small.RDS')
```
...
saveRDS(standata,file="./probsuccess-data-small.RDS")
```

```{r}
standata<-readRDS("./probsuccess-data-small.RDS")

cmdstanr::set_cmdstan_path("/home/teodor/.cmdstan/cmdstan-2.32.2")
file <- file.path(cmdstanr::cmdstan_path(),"probsuccess-small.stan")
...
probsuccess.fit$save_object(file = "./probsuccess-fit-small.RDS")
```
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex",
      table.envir = 'table',
      caption='Diagnostics for the posterior effects (small allocation of labels)', 
      booktabs=T,
      label='tab:diagnostics-small-probsuccess',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-small.tex')
```
...
saveRDS(tt,'./probsuccess-par-table-small.RDS')
```
Create table
```{r}
tt[c(1:34),c(1,5:7)] %>%
kable(
        "latex", 
        table.envir = 'table',
        caption='Odds ratios for fixed effects and noise parameters (small allocation of labels)', 
        booktabs=T,
        label='summarystatisticstable-OR-small',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-OR-small.tex')
```
...
png("traceplots-probsuccess-small.png")
p_trace
```
