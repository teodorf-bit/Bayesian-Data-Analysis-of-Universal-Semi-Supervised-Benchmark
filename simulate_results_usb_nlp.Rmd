...
## Import dataset, remove defixmath because it is only used for task == "usb_cv", therefore it cannot be used for the usb-nlp results.
```{r}
...
dataset%>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the computation error rate for usb-nlp data', 
        booktabs=T,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-error-rate-usb-nlp.tex')
```
...
pdf("boxplots-error-rate-usb-nlp.pdf")
#png("boxplots-error-rate-usb-nlp.png")
p
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error_rate for usb-nlp data', 
        booktabs=T,
        label='summarystatisticstable-usb-nlp',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-usb-nlp.tex')
```
...
saveRDS(df_out, file="./ranking-usb-nlp.RDS")
```

```{r pressure, echo=FALSE}
df_out <- readRDS("./ranking-usb-nlp.RDS")
...
saveRDS(standata, file = "./rankingmodel-data-usb-nlp.RDS")
```
CmdStanR
```{r}
standata<-readRDS("./rankingmodel-data-usb-nlp.RDS")
...
saveRDS(ranking.fit, file = "./ranking-fit-usb-nlp.RDS")
ranking.fit$save_object(file = "./ranking-fit-usb-nlp.RDS")

```
Read in the fitted data
```{r}
ranking.fit <-readRDS("./ranking-fit-usb-nlp.RDS")
...
summary_ranking.fit <- ranking.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (text datatype)', 
      booktabs=T,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-usb-nlp.tex')
```
...
#pdf('strength-usb-nlp.pdf')
png("strength-usb-nlp.png")
p_mcmc_intervals
```
...
#pdf('posterior-strength-parameters-usb-nlp.pdf')
png("posterior-strength-parameters-usb-nlp.png")
p_areas
```

```{r}
p_bars <- bayesplot::ppc_bars(y=y,yrep=as.matrix(y_rep))
#pdf('posterior-predictive-check-boxplot-usb-nlp.pdf')
png("posterior-predictive-check-boxplot-usb-nlp.png")
p_bars
```

```{r}
p_bars_grouped <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=df_out$algo0_name, binwidth = 0.005)
#pdf('posterior-predictive-check-bars_grouped-usb-nlp.pdf')
png("posterior-predictive-check-bars_grouped-usb-nlp.png")
p_bars_grouped
...
#pdf('posterior-predictive-check-rootogram-usb-nlp.pdf')
png("posterior-predictive-check-rootogran.usb-nlp.png")
p_rootogram
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms w.r.t text data', 
      booktabs=T,
      label='rankingusb-nlp',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('ranking-usb-nlp.tex')
```
