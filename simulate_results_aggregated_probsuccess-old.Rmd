...
probsuccess.fit <- stan(file = './probsuccess.stan',
                     ...
                     warmup = 200,
                     iter = 3000)
saveRDS(probsuccess.fit, file = "./probsuccsess-fit.RDS")
```
```{r}
probsuccess.fit <-readRDS("./probsuccsess-fit.RDS")
```

```{r}
summary(probsuccess.fit)$summary
```
P
```{r}
a_alg <- c("a_alg[1]",
...
p <- p_noise + plot_annotation(title = "HPDI interval#")
...
pars <- c(a_alg, b_noise, 's')
stanfit_summary <- as_tibble(summary(probsuccess.fit)$summary, rownames="Parameter")
t1 <- stanfit_summary %>%
...
colnames(t)<-c("Parameter", "Mean", "HPD low", "HPD high","\\hat{R}","$n_{eff}$",'OR Mean','OR low','OR HPD high')
# 
# saveRDS(t,'./probsuccess-par-table.RDS')
```
