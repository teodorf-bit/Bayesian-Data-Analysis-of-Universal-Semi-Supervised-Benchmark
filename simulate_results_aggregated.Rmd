...
dataset%>% 
  ...
  kable(
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-error-rate-aggregated.tex')
```
...
pdf("boxplots-error-rate.pdf")
#png("boxplots-error-rate.png")
p

```

## Summary statistics for datatype
```{r}
p<-ggplot(data=dataset, aes(x=algorithm, y=error_rate, fill=algorithm))+
  geom_boxplot()+
  theme(axis.text.x = element_blank())+ #remove the x labels
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error_rate for aggregated data', 
        booktabs=T,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-aggregated.tex')
```
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms w.r.t aggregated data', 
      booktabs=T,
