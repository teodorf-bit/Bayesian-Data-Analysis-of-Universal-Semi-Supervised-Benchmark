...
probsuccess.fit$save_object(file = "./probsuccess-fit-usb-cv.RDS")
```
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (image datatype)', 
      booktabs=T,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-usb-cv.tex')
```
...
saveRDS(tt,'./probsuccess-par-table-usb-cv.RDS')
```
Create table
```{r}
tt[c(1:34),c(1,5:7)] %>%
kable(
        "latex", 
        table.envir = 'table',
        caption='Odds ratios for fixed effects and noise parameters for image data', 
        booktabs=T,
...
png("traceplots-probsuccess-usb-cv.png")
p_trace
```
