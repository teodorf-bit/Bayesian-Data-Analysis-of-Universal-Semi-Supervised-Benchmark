...
## Import dataset, remove defixmath because it is only used for task == "usb_cv", therefore it cannot be used for the label = "large results.
```{r}
...
dataset%>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the computation error rate for the data when having a large allocation of labels', 
        booktabs=T,
        label='summarystatisticstable-large',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-error-rate-large.tex')
```
...
pdf("boxplots-error-rate-large.pdf")
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error_rate for large data', 
        booktabs=T,
        label='summarystatisticstable-large',
        format.args = list(scientific = FALSE), 
        digits = 3,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-large.tex')
```
...
saveRDS(df_out, file="./ranking-large.RDS")
```

```{r pressure, echo=FALSE}
df_out <- readRDS("./ranking-large.RDS")
...
saveRDS(standata, file = "./rankingmodel-data-large.RDS")
```
CmdStanR
```{r}
standata<-readRDS("./rankingmodel-data-large.RDS")
...
saveRDS(ranking.fit, file = "./ranking-fit-large.RDS")
ranking.fit$save_object(file = "./ranking-fit-large.RDS")

```
Read in the fitted data
```{r}
ranking.fit <-readRDS("./ranking-fit-large.RDS")
...
summary_ranking.fit <- ranking.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (large allocation)', 
      booktabs=T,
      label='tab:diagnostics-large',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-large.tex')
```
...
#pdf("parcoord-large.pdf")
png("parcoord-large.png")
p_parcoord
```
Kernel density estimates
```{r}
p_dense_overlay <-bayesplot::ppc_dens_overlay(y,as.matrix(y_rep))
#pdf("dense_overlay-large.pdf")
png("dense_overlay-large.png")
p_dense_overlay
```
...
#pdf('strength-large.pdf')
png("strength-large.png")
p_mcmc_intervals
```
...
#pdf('posterior-strength-parameters-large.pdf')
png("posterior-strength-parameters-large.png")
p_areas
```

```{r}
p_bars <- bayesplot::ppc_bars(y=y,yrep=as.matrix(y_rep))
#pdf('posterior-predictive-check-boxplot-large.pdf')
png("posterior-predictive-check-boxplot-large.png")
p_bars
```

```{r}
p_bars_grouped <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=df_out$algo0_name, binwidth = 0.005)
#pdf('posterior-predictive-check-bars_grouped-large.pdf')
png("posterior-predictive-check-bars_grouped-large.png")
p_bars_grouped
...
#pdf('posterior-predictive-check-rootogram-large.pdf')
png("posterior-predictive-check-rootogran.large.png")
p_rootogram
...
rank_df_table %>% 
  kable( 
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms for large allocation of labeled data', 
      booktabs=T,
      label='rankinglarge',
      format.args = list(scientific = FALSE), 
      digits = 3,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('ranking-large.tex')
```
