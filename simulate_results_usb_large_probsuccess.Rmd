...
## Import dataset, remove defixmath because it is only used for task == "usb_large", therefore it cannot be used for the aggregated results.
```{r}
...
```{r include=F}
saveRDS(dplyr::sample_n(dataset,size=4),'./probsuccessmodeldata-large.RDS')
```
...
saveRDS(standata,file="./probsuccess-data-large.RDS")
```

```{r}
standata<-readRDS("./probsuccess-data-large.RDS")

cmdstanr::set_cmdstan_path("/home/teodor/.cmdstan/cmdstan-2.32.2")
file <- file.path(cmdstanr::cmdstan_path(),"probsuccess-large.stan")
...
probsuccess.fit$save_object(file = "./probsuccess-fit-large.RDS")
```
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior effects (large allocation of labels)', 
      booktabs=T,
      label='tab:diagnostics-large-probsuccess',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-large.tex')
```
...
saveRDS(tt,'./probsuccess-par-table-large.RDS')
```
Create table
```{r}
tt[c(1:34),c(1,5:7)] %>%
kable(
        "latex", 
        table.envir = 'table',
        caption='Odds ratios for fixed effects and noise parameters (large allocation of labels)', 
        booktabs=T,
        label='summarystatisticstable-OR-large',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-OR-large.tex')
```
...
png("traceplots-probsuccess-large.png")
p_trace
```
