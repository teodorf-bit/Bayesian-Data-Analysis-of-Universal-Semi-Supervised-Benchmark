...
## Import dataset, remove defixmath because it is only used for task == "usb_cv", therefore it cannot be used for the usb-cv results.
```{r}
...
dataset%>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the computation error rate for usb-cv data', 
        booktabs=T,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-error-rate-usb-cv.tex')
```
...
pdf("boxplots-error-rate-usb-cv.pdf")
#png("boxplots-error-rate-usb-cv.png")
p
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error_rate for usb-cv data', 
        booktabs=T,
        label='summarystatisticstable-usb-cv',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-usb-cv.tex')
```
...
saveRDS(df_out, file="./ranking-usb-cv.RDS")
```

```{r pressure, echo=FALSE}
df_out <- readRDS("./ranking-usb-cv.RDS")
...
saveRDS(standata, file = "./rankingmodel-data-usb-cv.RDS")
```
CmdStanR
```{r}
standata<-readRDS("./rankingmodel-data-usb-cv.RDS")
...
saveRDS(ranking.fit, file = "./ranking-fit-usb-cv.RDS")
ranking.fit$save_object(file = "./ranking-fit-usb-cv.RDS")

```
Read in the fitted data
```{r}
ranking.fit <-readRDS("./ranking-fit-usb-cv.RDS")
...
summary_ranking.fit <- ranking.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (image datat)', 
      booktabs=T,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-usb-cv.tex')
```
...
#pdf("parcoord-usb-cv.pdf")
png("parcoord-usb-cv.png")
p_parcoord
```
Kernel density estimates
```{r}
p_dense_overlay <-bayesplot::ppc_dens_overlay(y,as.matrix(y_rep))
#pdf("dense_overlay-usb-cv.pdf")
png("dense_overlay-usb-cv.png")
p_dense_overlay
```
...
#pdf('strength-usb-cv.pdf')
png("strength-usb-cv.png")
p_mcmc_intervals
```
...
#pdf('posterior-strength-parameters-usb-cv.pdf')
png("posterior-strength-parameters-usb-cv.png")
p_areas
```

```{r}
p_bars <- bayesplot::ppc_bars(y=y,yrep=as.matrix(y_rep))
#pdf('posterior-predictive-check-boxplot-usb-cv.pdf')
png("posterior-predictive-check-boxplot-usb-cv.png")
p_bars
```

```{r}
p_bars_grouped <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=df_out$algo0_name, binwidth = 0.005)
#pdf('posterior-predictive-check-bars_grouped-usb-cv.pdf')
png("posterior-predictive-check-bars_grouped-usb-cv.png")
p_bars_grouped
...
#pdf('posterior-predictive-check-rootogram-usb-cv.pdf')
png("posterior-predictive-check-rootogran.usb-cv.png")
p_rootogram
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms w.r.t image data', 
      booktabs=T,
      label='rankingusb-cv',
      format.args = list(scientific = FALSE), 
      digits = 3,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('ranking-usb-cv.tex')
```
