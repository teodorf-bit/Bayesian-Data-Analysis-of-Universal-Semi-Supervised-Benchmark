...
## Import dataset, remove defixmath because it is only used for task == "usb_nlp", therefore it cannot be used for the aggregated results.
```{r}
...
```{r include=F}
saveRDS(dplyr::sample_n(dataset,size=4),'./probsuccessmodeldata-usb-nlp.RDS')
```
...
saveRDS(standata,file="./probsuccess-data-usb-nlp.RDS")
```

```{r}
standata <- readRDS("./probsuccess-data-usb-nlp.RDS")

cmdstanr::set_cmdstan_path("/home/teodor/.cmdstan/cmdstan-2.32.2")
file <- file.path(cmdstanr::cmdstan_path(),"probsuccess-nlp.stan")
...
probsuccess.fit$save_object(file = "./probsuccess-fit-usb-nlp.RDS")
```
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior effects (text datatype)', 
      booktabs=T,
  ...
  kable_styling(latex_options = c("hold_position"),
              full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-usb-nlp.tex')
```
...
saveRDS(tt,'./probsuccess-par-table-usb-nlp.RDS')
...
kable(
        "latex", 
        table.envir = 'table',
        caption='Odds ratios for fixed effects and noise parameters for text data', 
        booktabs=T,
        label='summarystatisticstable-OR-nlp',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-OR-nlp.tex')
```
...
png("traceplots-probsuccess-usb-nlp.png")
p_trace
```
