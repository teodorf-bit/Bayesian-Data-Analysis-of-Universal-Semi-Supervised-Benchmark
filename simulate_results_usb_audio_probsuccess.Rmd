...
## Import dataset, remove defixmath because it is only used for task == "usb_audio", therefore it cannot be used for the aggregated results.
```{r}
...
```{r include=F}
saveRDS(dplyr::sample_n(dataset,size=4),'./probsuccessmodeldata-audio-usb.RDS')
```
...
saveRDS(standata,file="./probsuccess-data-usb-audio.RDS")
```


```{r}
standata<-readRDS("./probsuccess-data-usb-audio.RDS")

cmdstanr::set_cmdstan_path("/home/teodor/.cmdstan/cmdstan-2.32.2")
file <- file.path(cmdstanr::cmdstan_path(),"probsuccess-audio.stan")
...
probsuccess.fit$save_object(file = "./probsuccess-fit-usb-audio.RDS")
```
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior effects (audio datatype)', 
      booktabs=T,
      label='tab:diagnostics-audio-probsuccess',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-usb-audio.tex')
```
...
saveRDS(tt,'./probsuccess-par-table-usb-audio.RDS')
```
Create table
```{r}
tt[c(1:34),c(1,5:7)] %>%
kable(
        "latex", 
        table.envir = 'table',
        caption='Odds ratios for fixed effects and noise parameters for audio data', 
        booktabs=T,
        label='summarystatisticstable-OR-audio',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-OR-audio.tex')
```
...
png("traceplots-probsuccess-usb-audio.png")
p_trace
```
