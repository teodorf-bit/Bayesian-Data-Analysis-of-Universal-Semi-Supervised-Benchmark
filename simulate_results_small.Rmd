...
## Import dataset, remove defixmath because it is only used for task == "usb_cv", therefore it cannot be used for the label = "small results.
```{r}
...
dataset%>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the computation error rate for the data when having a small allocation of labels', 
        booktabs=T,
        label='summarystatisticstable-small',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-error-rate-small.tex')
```
...
pdf("boxplots-error-rate-small.pdf")
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error_rate for small data', 
        booktabs=T,
        label='summarystatisticstable-small',
        format.args = list(scientific = FALSE), 
        digits = 3,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-small.tex')
```
...
saveRDS(df_out, file="./ranking-small.RDS")
```

```{r pressure, echo=FALSE}
df_out <- readRDS("./ranking-small.RDS")
...
saveRDS(standata, file = "./rankingmodel-data-small.RDS")
```
CmdStanR
```{r}
standata<-readRDS("./rankingmodel-data-small.RDS")
...
saveRDS(ranking.fit, file = "./ranking-fit-small.RDS")
ranking.fit$save_object(file = "./ranking-fit-small.RDS")

```
Read in the fitted data
```{r}
ranking.fit <-readRDS("./ranking-fit-small.RDS")
...
summary_ranking.fit <- ranking.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (small allocation)', 
      booktabs=T,
      label='tab:diagnostics-small',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-small.tex')
```
...
#pdf("parcoord-small.pdf")
png("parcoord-small.png")
p_parcoord
```
Kernel density estimates
```{r}
p_dense_overlay <-bayesplot::ppc_dens_overlay(y,as.matrix(y_rep))
#pdf("dense_overlay-small.pdf")
png("dense_overlay-small.png")
p_dense_overlay
```
...
#pdf('strength-small.pdf')
png("strength-small.png")
p_mcmc_intervals
```
...
#pdf('posterior-strength-parameters-small.pdf')
png("posterior-strength-parameters-small.png")
p_areas
```

```{r}
p_bars <- bayesplot::ppc_bars(y=y,yrep=as.matrix(y_rep))
#pdf('posterior-predictive-check-boxplot-small.pdf')
png("posterior-predictive-check-boxplot-small.png")
p_bars
```

```{r}
p_bars_grouped <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=df_out$algo0_name, binwidth = 0.005)
#pdf('posterior-predictive-check-bars_grouped-small.pdf')
png("posterior-predictive-check-bars_grouped-small.png")
p_bars_grouped
...
#pdf('posterior-predictive-check-rootogram-small.pdf')
png("posterior-predictive-check-rootogran.small.png")
p_rootogram
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms for small allocation of labeled data', 
      booktabs=T,
      label='rankingsmall',
      format.args = list(scientific = FALSE), 
      digits = 3,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('ranking-small.tex')
```
